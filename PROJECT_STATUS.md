...
- Existing `DeepFilterNetEnhancer.load()` currently mismatches installed `df.init_df` 3-tuple return (pre-existing env issue); experiment bypasses that path intentionally
...
3. **If revisiting noise-aware enhancement** — train/evaluate the classifier on speech+noise mixtures and/or learn category-specific policies that can beat full DF3; do not ship the current `atten_lim_db` map.
